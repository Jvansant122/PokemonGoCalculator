 ...
 *
 * As of 2026-09-10 the "Outperforms means..."/"Honest limitation..."
 * methodology paragraphs live in a collapsed `<details className="prose-
 * details">` below the verdict/table (Species Report's own result-panel
 * pattern, see SpeciesReportView.tsx's "How ... are actually computed"
 * toggle) — the finding (verdict sentence + breakdown table) stays visible,
 * only the paragraph-length justification for it is tucked away. Text is
 * unchanged/unsoftened from before this move, including the "Honest
 * limitation" framing — that hedging is deliberate, not decoration.
 */
export function IvSweepReport({ sweepAggregate, ivA, ivB }: Props) {
  return (
    <CollapsibleSection id="iv-breakpoints-sweep-report" heading="Impact across every raid target this tool can model" defaultOpen>
      ...
      <details className="prose-details" style={{ marginTop: 12 }}>
        <summary>How "outperforms" is scored, and what the tier-4+ restriction excludes</summary>
        <p>
          ...
          that has ever existed — see the "Known caveats" section below.
