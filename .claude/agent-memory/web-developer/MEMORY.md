...
- [IV Breakpoints: active-boss sweep report + reversed table](feature_iv_breakpoints_active_boss_sweep.md) — formatDivergentLevels run-index compaction, shared resolvedAttackerMoves, sweep uses each boss's own first fast move not the single-target picker
