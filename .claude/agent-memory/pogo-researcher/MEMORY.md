...
- [Fact: TM / Elite TM move-change mechanics](fact_tm_move_change_mechanics.md) — 4 items only; regular=random+excludes legacy, Elite=chosen+includes legacy; uniformity NOT Niantic-confirmed, contested; Elite scarce (0-3 typical)
- [Fact: Frustration removal is event-gated](fact_frustration_removal_event_gating.md) — no TM (regular/Elite) works except ~quarterly "Taken Over" events; purify is always-available+ungated; permanent once changed
- [Fact: Mega/Primal have no separate movepool](fact_mega_moveset_inheritance.md) — tempEvoOverrides carries only stats/type/energy, first-party GAME_MASTER proof (Blaziken, Rayquaza); TMing base = TMing mega
- [Fact: Poke Genie blank move field = unknown, not moveless](fact_pokegenie_blank_move_field_meaning.md) — every real Pokemon always has moves; blank CSV cell means uncaptured, not "needs a TM" — corrects a proposed framing
- [Proposal: move-change as optimizer candidates](proposal_move_change_optimizer_candidates.md) — EV-band for random TMs, deterministic Elite TM/2nd-move, moveset-unknown 3rd state, Frustration-event caveat; pending, flags new Scenario field
- [Fact: dodge 500ms vs 700ms reconciled](fact_dodge_window_500_vs_700ms_reconciled.md) — round 5 (2026-09-10): different quantities (invulnerability vs reaction window), no engine change
- [Fact: Super Max eligibility confidence upgraded](fact_super_max_eligibility_upgraded_round5.md) — round 5: first-party Niantic post + 2 community sites corroborate the "+"-move roster; engine gate unchanged, correct
- [Fact: round-5 accuracy pass negative results](fact_accuracy_pass_round5_negative_results.md) — boss-AI denominator, holdChargedMoveUntilSafe cost, dodge-HP-scaling, damage-formula-incomplete, +10%/tier all still unsourceable; caught 1 WebSearch fabrication; nothing new since 2026-09-09 breaks engine math
- [Proposal: Shadow Raid enrage simulation](proposal_shadow_raid_enrage_simulation.md) — ideation 2026-09-10; 60%-15% HP boss stat escalation, no engine code exists at all (verified via grep); strongest of 3 proposals — pending
- [Proposal: Team Raid boss-moveset sweep](proposal_team_raid_moveset_sweep.md) — ideation 2026-09-10; extends Comparator-only compareAcrossBossChargedMoves to Team Raid Simulator; not a dup of the Comparator proposal — pending
- [Proposal: Mega Energy investment, NOT recommended](proposal_mega_energy_investment_not_recommended.md) — ideation 2026-09-10; blocked on 2 stacked unverified magnitudes, explicitly don't build until sourced
- [Fact: raid footage review — Dynamic Punch+ number NOT recoverable](fact_raid_footage_review_dynamic_punch_plus.md) — client shows no damage numbers/Mega Level badge; HP-bar back-calc declined as too compounded; zero contradictions found
