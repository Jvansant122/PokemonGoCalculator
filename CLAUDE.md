...
- `packages/web` — Vite + React + TypeScript UI, imports `@pogo-analyzer/engine` straight from
  its TS source. Owned by `web-developer` for features, `site-builder` for build/deploy. Three
  ...
  **Comparator**, the **Team Raid Simulator** (one trainer's own 6-slot sequential roster vs. a
  boss's real HP pool and countdown timer, with wipe-and-revive looping), and the **Species
  Report** (one Pokémon ranked against every currently-active real raid boss). Each has its own
  shareable `Scenario`-family type and URL query param (`s` / `ts` / `sr`) — don't conflate them.
